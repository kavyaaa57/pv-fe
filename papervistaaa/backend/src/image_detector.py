...
def check_image_plagiarism(suspect_file_path, reference_image_paths):
    """Checks for visual plagiarism using Perceptual Hashing (pHash)."""
    results = []
    
    if not os.path.exists(suspect_file_path):
        return "⚠️ Suspect file not found. Skipping visual check."

    # Try to open suspect file as an image
    try:
        img_suspect = Image.open(suspect_file_path)
        hash_suspect = imagehash.phash(img_suspect, hash_size=IMAGE_HASH_SIZE)
    except Exception:
        # File is not a simple image, skip for this check
